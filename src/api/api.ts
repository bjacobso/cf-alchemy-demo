...
class WorkflowsGroup extends HttpApiGroup.make("workflows")
  ...
  .add(
    HttpApiEndpoint.get("getExecution")`/${executionIdParam}`.addSuccess(
      Schema.String
    )
  )
  .add(
    HttpApiEndpoint.post("sendEvent")`/${executionIdParam}/event`.addSuccess(
      Schema.String
    )
  )
  .add(
    HttpApiEndpoint.post(
      "cancelExecution"
    )`/${executionIdParam}/cancel`.addSuccess(Schema.String)
  ) {}
