...
export const DurableObjectWorkflowEngineLive = Layer.scoped(
  ...
  Effect.gen(function* () {
    ...
    const getStub = (executionId: string): DurableObjectStub<WorkflowExecution> => {
    ...
    const encoded: WorkflowEngine.Encoded = {
      ...
      execute: <const Discard extends boolean>(
        ...
        Effect.gen(function* () {
          ...
          if (!registered) {
            return yield* Effect.die(new Error(`Workflow "${workflow.name}" is not registered`));
          }
          ...
          const result: Workflow.Result<unknown, unknown> = yield* registered
            ...
            .pipe(
              ...
              Effect.catchAll((error) =>
                Effect.succeed(new Workflow.Complete({ exit: Exit.fail(error) })),
              ),
          ...
          if (result._tag === "Complete") {
            if (Exit.isSuccess(result.exit)) {
              ...
              yield* Effect.tryPromise(() => stub.fail(result.exit)).pipe(Effect.orDie);
            }
          } else {
      ...
      poll: (
        ...
        Effect.gen(function* () {
          ...
          const status: PollResult | undefined = yield* Effect.tryPromise(() => stub.poll()).pipe(
            Effect.orDie,
          );
      ...
      interrupt: (workflow: Workflow.Any, executionId: string): Effect.Effect<void> =>
        Effect.gen(function* () {
          const stub = getStub(executionId);
      ...
      resume: (workflow: Workflow.Any, executionId: string): Effect.Effect<void> =>
        Effect.gen(function* () {
          const stub = getStub(executionId);
      ...
      activityExecute: (
        ...
        attempt: number,
      ): Effect.Effect<Workflow.Result<unknown, unknown>, never, WorkflowEngine.WorkflowInstance> =>
        Effect.gen(function* () {
          const instance = yield* WorkflowEngine.WorkflowInstance;
          const stub = getStub(instance.executionId);

          // Check if activity already completed (replay)
          const existing = (yield* Effect.tryPromise(() => stub.getActivity(activity.name)).pipe(
            Effect.orDie,
          ...
          yield* Effect.tryPromise(() => stub.activityStart(activity.name, attempt)).pipe(
            Effect.orDie,
          );

          // Execute the activity
          const result: Workflow.Result<unknown, unknown> = yield* activity.executeEncoded.pipe(
            Effect.map((value) => new Workflow.Complete({ exit: Exit.succeed(value) })),
            Effect.catchAll((error) =>
              Effect.succeed(new Workflow.Complete({ exit: Exit.fail(error) })),
            ),
            Effect.catchAllDefect((defect) =>
              Effect.succeed(new Workflow.Complete({ exit: Exit.die(defect) })),
            ),
          );
          ...
          if (result._tag === "Complete") {
            if (Exit.isSuccess(result.exit)) {
              ...
              yield* Effect.tryPromise(() => stub.activityFail(activity.name, result.exit)).pipe(
                Effect.orDie,
              );
            }
          }
      ...
      > =>
        Effect.gen(function* () {
          ...
          const result = yield* Effect.tryPromise(() => stub.deferredResult(deferred.name)).pipe(
            Effect.orDie,
          );
          ...
          yield* Effect.tryPromise(() => stub.registerDeferred(deferred.name)).pipe(Effect.orDie);
      ...
      scheduleClock: (
        ...
        Effect.gen(function* () {
          ...
          yield* Effect.tryPromise(() => stub.scheduleClock(options.clock.name, wakeAt)).pipe(
            Effect.orDie,
          );
        }),
    };
